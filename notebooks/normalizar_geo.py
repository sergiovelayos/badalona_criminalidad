...
# # Eliminar registros que empiecen por "-" (con posibles espacios antes/después del guión)
# delitos = delitos[~delitos["geo"].str.match(r"^\s*-\s*.*", na=False)]

# # Reemplazar 'Municipio de ' por cadena vacía al inicio del texto
# delitos["geo"] = delitos["geo"].str.replace(r"^Municipio de ", "", regex=True)
...
# # Se usa 'unique' para evitar procesar duplicados
# for item in delitos["geo"].dropna().unique():  # Añadido dropna() para evitar valores NaN
#     match = re.match(r"^(\d{5})\s(.*)", str(item))  # Añadido str() por seguridad
#     if match:
#         codigo = match.group(1)
#         municipio = match.group(2)
...
# # Esta función realiza la unión (lookup)
# def unir_municipio_a_cp(fila):
#     # Manejar valores NaN
#     if pd.isna(fila):
#         return fila
...
#     # La fila ya tiene el formato completo (empieza con 5 dígitos)
#     if re.match(r"^\d{5}", fila_str):
#         return fila
    
#     # La fila es solo el municipio, buscar en el diccionario
#     elif fila_str in municipio_a_codigo:
#         codigo = municipio_a_codigo[fila_str]
#         return f"{codigo} {fila_str}"
    
#     # No se encuentra en el diccionario, devolver el valor original
#     else:
#         return fila

# # Guardar el conteo original para verificación
# conteo_original = len(delitos)
# valores_originales = delitos["geo"].value_counts()

# # Aplicar la función a la columna 'geo'
# delitos["geo"] = delitos["geo"].apply(unir_municipio_a_cp)

# filter = delitos["geo"].str.contains("Badalona")

# print(delitos.loc[filter,'año'].unique())

# # delitos.to_csv("./data/check_borrar.csv", index=False, encoding="utf-8")
