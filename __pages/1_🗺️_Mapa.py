...
def load_optimized_crime_data():
    ...
    df.drop(columns=['valor_acumulado', 'pob', 'periodo_dt', 'quarter', 'year'], inplace=True)
...
def prepare_map_data(_gdf, df_filtered, tipo_geo, source_column):
    ...
    gdf_merged = pd.merge(_gdf, df_filtered[df_filtered['tipo_geo'] == tipo_geo],
                          left_on='JOIN_CODE', right_on='codigo_geo', how='left')
    ...
    datetime_columns = gdf_merged.select_dtypes(include=['datetime64']).columns.tolist()
    if datetime_columns:
        gdf_merged = gdf_merged.drop(columns=datetime_columns)
...
st.markdown("""
Explora la criminalidad en España con este mapa interactivo. Usa los filtros para analizar los datos por tipo de delito, periodo y métrica.
...
if not df_crime.empty:
    ...
    if gdf_final is not None:
        ...
        if mapa:
            ...
            try:
                st_folium(mapa, height=map_height)
            except Exception as e:
                st.info("Renderizando mapa en modo alternativo...")
